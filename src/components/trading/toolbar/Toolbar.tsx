...
export default function Toolbar() {
  ...
  const setLiveWsUrl = useAppStore((s) => s.setLiveWsUrl);
  ...
  const handleTimeframeChange = useCallback(async (tf: Timeframe) => {
    ...
    try {
      ...
      const isLive = useAppStore.getState().liveMode;
      const endpoint = isLive
        ? `/api/live/bars?symbol=${encodeURIComponent(sym)}&timeframe=${tf}&limit=500`
        : `/api/bars?symbol=${encodeURIComponent(sym)}&timeframe=${tf}&limit=5000`;
  ...
  const handleLiveToggle = useCallback(async () => {
    ...
    if (nextLive) {
      // Fetch WS config first
      try {
        const res = await fetch(`/api/live/config?symbol=${encodeURIComponent(symbol)}`);
        const config = await res.json();
        if (config.error) {
          addToast(config.error, 'error');
          return;
        }
        setLiveWsUrl(config.wsUrl);
        setLiveMode(true);
        // Reload bars from live endpoint
        const barsRes = await fetch(`/api/live/bars?symbol=${encodeURIComponent(symbol)}&timeframe=${timeframe}&limit=500`);
        if (barsRes.ok) {
          const json = await barsRes.json();
          if (json.data && Array.isArray(json.data)) {
            useChartStore.getState().setBars(json.data);
            useChartStore.getState().setCurrentBarIndex(json.data.length - 1);
          }
        }
        addToast('Live mode enabled', 'success');
      } catch (err) {
        addToast('Failed to connect to live data', 'error');
      }
    } else {
      setLiveMode(false);
      setLiveWsUrl(null);
      // Reload bars from sim endpoint
      const barsRes = await fetch(`/api/bars?symbol=${encodeURIComponent(symbol)}&timeframe=${timeframe}&limit=5000`);
      if (barsRes.ok) {
        const json = await barsRes.json();
        if (json.data && Array.isArray(json.data)) {
          useChartStore.getState().setBars(json.data);
      ...
      addToast('Simulation mode', 'info');
    }
  }, [liveMode, symbol, timeframe, setLiveMode, setLiveWsUrl, addToast]);
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            ...
            size="sm"
            className={cn(
              'h-7 gap-1 text-[10px] font-bold tracking-wider shrink-0 transition-all',
              liveMode
                ? liveConnected
                  ? 'text-amber-400'
                  : 'text-amber-600'
            ...
            style={liveMode && liveConnected ? { background: 'rgba(245,158,11,0.15)', boxShadow: '0 0 8px rgba(245,158,11,0.3)' } : undefined}
            disabled={!symbolHasLive}
        ...
        <TooltipContent side="bottom">
          <p>{liveMode ? 'Switch to Simulation' : 'Switch to Live Data'}</p>
        </TooltipContent>
      </Tooltip>
